...
mod tests {
    ...
    async fn translation_is_human_only_bounded_and_never_exposes_key() {
        ...
        let unauth = app.clone().oneshot(Request::get("/api/v1/translation/settings")
        ...
        let saved = app.clone().oneshot(Request::put("/api/v1/translation/settings")
        ...
        let loaded = app.clone().oneshot(Request::get("/api/v1/translation/settings")
        ...
        for (method, path, body) in [
            ...
            ("POST", "/api/v1/translation/translate", r#"{"text":"Hello","targetLanguage":"French"}"#),
        ] {
            let res = app.clone().oneshot(Request::builder().method(method).uri(path)
        ...
        for (text, expected) in [(String::new(), StatusCode::BAD_REQUEST),
            ("x".repeat(32 * 1024 + 1), StatusCode::BAD_REQUEST),
            ("x".repeat(256 * 1024), StatusCode::PAYLOAD_TOO_LARGE)] {
            let res = app.clone().oneshot(Request::post("/api/v1/translation/translate")
                .header("authorization", "Bearer tok").header("content-type", "application/json")
                .body(Body::from(serde_json::json!({"text":text,"targetLanguage":"French"}).to_string())).unwrap()).await.unwrap();
