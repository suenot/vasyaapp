...
- **Cookie session auth + CSRF (server side, for the web build).** `vasya-server` now also accepts the human session token from an HttpOnly `vasya_token` cookie when no `Authorization` header is present, and enforces a double-submit CSRF token (`x-csrf-token` header must match the `vasya_csrf` cookie) on cookie-authenticated state-changing requests. Bearer/agent-key auth is unchanged (not CSRF-vulnerable). This lets the web build move the JWT out of `localStorage` (cookie issuance + frontend cutover follow). Agent keys never authenticate via cookie.
